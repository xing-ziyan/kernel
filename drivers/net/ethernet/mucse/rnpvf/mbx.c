...
#define PF_VF_SHM(mbx, vf)      \
	((mbx->pf_vf_shm_base) + (64 * (vf))) //for PF1 rtl will remap 6000 to 0xb000
...
static inline void rnpvf_mbx_inc_vfreq(struct rnpvf_hw *hw, bool to_cm3)
{
	...
	int reg = to_cm3 ? VF2CPU_COUNTER(mbx, vfnum) : VF2PF_COUNTER(mbx, vfnum);
...
static s32 rnpvf_check_for_msg_vf(struct rnpvf_hw *hw, bool to_cm3)
{
	...
	if (to_cm3 == true) {
		if (rnpvf_mbx_get_req(hw, CPU2VF_COUNTER(mbx, vfnum)) != hw->mbx.cpu_req) {
			ret_val = 0;
			hw->mbx.stats.reqs++;
		}
	} else {
		...
		// hw->mbx.pf_req);
		if (rnpvf_mbx_get_req(hw, PF2VF_COUNTER(mbx, vfnum)) != hw->mbx.pf_req) {
			ret_val = 0;
			hw->mbx.stats.reqs++;
...
static s32 rnpvf_check_for_ack_vf(struct rnpvf_hw *hw, bool to_cm3)
{
	...
	if (to_cm3 == true) {
		if (rnpvf_mbx_get_ack(hw, CPU2VF_COUNTER(mbx, vfnum)) != hw->mbx.cpu_ack) {
			ret_val = 0;
			hw->mbx.stats.acks++;
		}
	} else {
		if (rnpvf_mbx_get_ack(hw, PF2VF_COUNTER(mbx, vfnum)) != hw->mbx.pf_ack) {
			ret_val = 0;
			hw->mbx.stats.acks++;
		}
	}
...
static s32 rnpvf_obtain_mbx_lock_vf(struct rnpvf_hw *hw, bool to_cm3)
{
	...
	struct rnpvf_adapter *adapter = hw->back;
	u32 CTRL_REG =
		(to_cm3) ? VF2CPU_MBOX_CTRL(mbx, vfnum) : VF2PF_MBOX_CTRL(mbx, vfnum);
			      ...
			      bool to_cm3)
{
	...
	u8 vfnum = VFNUM(mbx, hw->vfnum);
	u32 DATA_REG =
		(to_cm3) ? CPU_VF_SHM_DATA(mbx, vfnum) : PF_VF_SHM_DATA(mbx, vfnum);
	u32 CTRL_REG =
		(to_cm3) ? VF2CPU_MBOX_CTRL(mbx, vfnum) : VF2PF_MBOX_CTRL(mbx, vfnum);
			     ...
			     bool to_cm3)
{
	...
	u32 i;
	u8 vfnum = VFNUM(mbx, hw->vfnum);
	u32 BUF_REG = (to_cm3) ? CPU_VF_SHM_DATA(mbx, vfnum) : PF_VF_SHM_DATA(mbx, vfnum);
	u32 CTRL_REG =
		(to_cm3) ? VF2CPU_MBOX_CTRL(mbx, vfnum) : VF2PF_MBOX_CTRL(mbx, vfnum);
