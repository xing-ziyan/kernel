...
void rnpvf_set_veb_mac_n500(struct rnpvf_hw *hw, u8 *mac, u32 vf_num, u32 ring)
{
	u32 maclow, machi;
	...
	wr32(hw, RNP_DMA_PORT_VBE_MAC_LO_TBL_N500, maclow);
	wr32(hw, RNP_DMA_PORT_VBE_MAC_HI_TBL_N500, machi);

	//ring = adapter->rx_ring[0]->rnpvf_queue_idx;
	//ring |= ((0x80 | vfnum) << 8);
	wr32(hw, RNP_DMA_PORT_VEB_VF_RING_TBL_N500, ring);


}


void rnpvf_set_vlan_n500(struct rnpvf_hw *hw, u16 vid, u32 vf_num)
{

	wr32(hw, RNP_DMA_PORT_VEB_VID_TBL_N500, vid);

} 



static const struct rnpvf_hw_operations rnpvf_hw_ops_n500 = {
	.set_veb_mac = rnpvf_set_veb_mac_n500,
	.set_veb_vlan = rnpvf_set_vlan_n500,
};

void rnpvf_set_veb_mac_n10(struct rnpvf_hw *hw, u8 *mac, u32 vfnum, u32 ring)
{
	int port; 
	u32 maclow, machi;
	...
	for (port = 0; port < 4; port++) {
		...
		wr32(hw, RNP_DMA_PORT_VBE_MAC_LO_TBL_N10(port, vfnum), maclow);
		wr32(hw, RNP_DMA_PORT_VBE_MAC_HI_TBL_N10(port, vfnum), machi);

		//ring = adapter->rx_ring[0]->rnpvf_queue_idx;
		//ring |= ((0x80 | vfnum) << 8);
		wr32(hw, RNP_DMA_PORT_VEB_VF_RING_TBL_N10(port, vfnum), ring);
	}
...
static s32 rnpvf_get_invariants_n500(struct rnpvf_hw *hw)
{
	struct rnp_mbx_info *mbx = &hw->mbx;
	...
	memcpy(&hw->ops, &rnpvf_hw_ops_n500, sizeof(hw->ops));
...
const struct rnpvf_info rnp_n500_vf_info = {
	.mac = rnp_mac_2port_40G,
	.mac_ops = &rnpvf_mac_ops,
	.board_type = rnp_board_n500,
	.get_invariants = &rnpvf_get_invariants_n500,
};
