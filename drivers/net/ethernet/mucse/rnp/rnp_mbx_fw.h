		...
		int fw_bytes)
{
	...
	req->reply_hi = 0;
	req->fw_update.paration = partition;
	req->fw_update.bytes = fw_bytes; // n500 use this as offset
	//req->fw_update.bin_phy_lo = fw_bin_phy_lo; //no use
	//req->fw_update.bin_phy_hi = fw_bin_phy_hi;
...
static inline void
build_phy_pause_set(struct mbx_fw_cmd_req *req, int pause_mode, int nr_lane)
{
	req->flags = 0;
...
static inline void
build_phy_link_set(struct mbx_fw_cmd_req *req, unsigned int adv, int nr_lane,
	unsigned int autoneg, unsigned int speed, unsigned int duplex, unsigned int tp_mdix_ctrl)
{
	req->flags = 0;
...
static inline void build_mbx_wol_set(
	struct mbx_fw_cmd_req *req, unsigned nr_lane, u32 mode)
{
	req->flags = 0;
	req->opcode = SET_WOL;
	req->datalen = sizeof(req->sfp_write);
