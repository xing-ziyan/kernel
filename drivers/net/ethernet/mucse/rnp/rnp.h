...
#define RNP_PAGE_BUFFER_NUMS(ring)  ((1 << RNP_ALLOC_PAGE_ORDER) * PAGE_SIZE / ALIGN((rnp_rx_offset(ring) + rnp_rx_bufsz(ring) + SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) + RNP_RX_HWTS_OFFSET), 1024))
...
#define RNP_MAX_PF_MACVLANS_N500 15
...
extern int
rnp_wol_supported(struct rnp_adapter *adapter, u16 device_id, u16 subdevice_id);
extern void rnp_clear_interrupt_scheme(struct rnp_adapter *adapter);
extern netdev_tx_t
rnp_xmit_frame_ring(struct sk_buff *, struct rnp_adapter *, struct rnp_ring *, bool);
...
#endif
extern int rnp_setup_tx_maxrate(struct rnp_ring *tx_ring,
								u64 max_rate,
								int sample_interval);
...
extern s32 rnp_fdir_erase_perfect_filter(int fdir_mode,
		struct rnp_hw *hw,
		union rnp_atr_input *input,
		u16 hw_id);
...
extern s32
rnp_enable_rxr_maxrate(struct net_device *netdev, u8 queue_index, u32 maxrate);
...
int rnp500_fw_update(struct rnp_hw *hw,
				  int partition,
				  const u8 *fw_bin,
				  int bytes);

int rnp_fw_update(struct rnp_hw *hw,
				  int partition,
				  const u8 *fw_bin,
				  int bytes);
