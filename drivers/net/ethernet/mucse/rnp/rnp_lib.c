...
__maybe_unused static void rnp_get_first_reg_idx(struct rnp_adapter *adapter, u8 tc,
...
static bool rnp_cache_ring_rss(struct rnp_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		ring->ring_addr = dma->dma_ring_addr + RING_OFFSET(ring->rnp_queue_idx);
				  ...
				  u8 queue, bool prio_flag)
{
	if (filter->formatted.flow_type == RNP_ATR_FLOW_TYPE_ETHER) {
		hw->ops.set_layer2_remapping(hw, filter, hw_id, queue, prio_flag);
		//rnp_setup_layer2_remapping(hw, filter, hw_id, queue);
	} else {
		hw->ops.set_tuple5_remapping(hw, filter, hw_id, queue, prio_flag);
